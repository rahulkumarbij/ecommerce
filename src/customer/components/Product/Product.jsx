...
export default function Product() {
  ...
  const searchPramms = new URLSearchParams(decodedQueryString);
  const colorValue = searchPramms.get("color");
  const sizeValue = searchPramms.get("size");
  const priceValue = searchPramms.get("price");
  const discount = searchPramms.get("discount");
  const sortValue = searchPramms.get("sort");
  const pageNumber = searchPramms.get("page") || 1;
  const stock = searchPramms.get("stock");
  ...
  const handleRadioFilterChange = (e, sectionId) => {
    const searchParamms = new URLSearchParams();
